...
trait LowPriorityFramework { self: Framework =>
  ...
  /**
    ...
    * when the Rx changes.
    */
  implicit def RxStr[T](r: Rx[T])(implicit f: T => Frag,
                                  ctx: Ctx.Owner): Frag = {
    RxHtmlTag(Rx(span(r())))
  }
}
...
trait Framework extends LowPriorityFramework {
  ...
  implicit def RxAttrValue[T: AttrValue](implicit ctx: Ctx.Owner) =
    new AttrValue[Rx.Dynamic[T]] {
      def apply(t: Element, a: Attr, r: Rx.Dynamic[T]): Unit = {
        r.trigger { implicitly[AttrValue[T]].apply(t, a, r.now) }
      }
    }

  implicit def RxStyleValue[T: StyleValue](implicit ctx: Ctx.Owner) =
