...
trait Renderers {
  ...
  def verticalRenderer(
    ...
    def render[R <: RenderingContext](
      ...
      0 until 1000 foreach { i =>
        val y = yAxis.min + i * (yAxis.max - yAxis.min) / 1000.0
        val x = data(0)
        r.render(VectorRow(Vector(x, y), ""), xAxis, yAxis, ctx, tx)
      }
  ...
  def abline(
    ...
    (dataSourceFromRows(Seq(a -> b)), List(polynom(() => renderer)))

  def horizontal(
    y: Double,
    renderer: DataRenderer = line()
  ) =
    (dataSourceFromRows(Seq(y -> 0d)), List(polynom(() => renderer)))

  def vertical(
    x: Double,
    renderer: DataRenderer = line()
  ) =
    (dataSourceFromRows(Seq(x -> 0d)), List(verticalRenderer(() => renderer)))
