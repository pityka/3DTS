...
class SaddlePlotSpec extends FunSpec with Matchers {
  ...
  describe("plots") {
    ...
    it("plot gallery") {
      ...
      val colortest2 = xyplot(indexed(0 until colorList.size map (_.toDouble) toSeq) -> point(color = DiscreteColors(colorList.size), colorCol = 1, size = 10))()

      val gallery =
        group(
          bar1,
          bar2, boxpl2,
          colortest, group(colortest2, AlignTo.bottomRightCorner(TextBox("boo"), colortest2.bounds), FreeLayout), p6, p6b, empty2, xyplot(Seq(0d -> 0d, 1d -> 1d, 2d -> 2d))(), r1, hist1, contour, density1, fig0, fig1, densmap, fig2, fig3, fig4, barplot2,
          TableLayout(3)
        )
