...
object HeptamerHelpers {
   ...
   def heptamerAt(cp: ChrPos, genome: IndexedFastaSequenceFile) : String= {
      ...
      genome.getSubsequenceAt(chr,bp0.toLong - 3,bp0.toLong+3).getBaseString().toUpperCase
   }
   ...
   def readFrequencies(source: scala.io.Source) : Map[String,Double] = source.getLines.map{ i =>
